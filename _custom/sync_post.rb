  ...
  class SyncPost
	...
	def initialize (passwd = nil)
	  ...
	  if passwd.nil?
		@globalConfig["MetaWeblog"].each do |site, paras|
		  puts 'Password of ' + site + ' is required.'
		  puts 'Username: ' + paras['MetaWeblog_username']
		  @passwdHash[site] = get_password("Enter your password here: ")
		  puts
		end
	  end
	end
	...
	def postBlog blogPath
	  ...
	  @globalConfig["MetaWeblog"].each do |site, paras|
		...
		if @passwd.nil?
		  password = @passwdHash[site]
		else
		  password = @passwd
		end
		blogClient = MetaWeblog::Client.new paras['MetaWeblog_url'], paras['MetaWeblog_blogid'].to_s, paras['MetaWeblog_username'], password, nil
